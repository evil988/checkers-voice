...
def show_menu():
    ...
    while True:
        ...
        for idx, txt in enumerate(textos, 1):
            surf = fonte.render(txt, True, colors[idx-1])
...
class VoiceMouseControlledCheckers:
    ...
    def draw_board(self):
        ...
        for y in range(8):
            for x in range(8):
                cor = WHITE if (x+y)%2==0 else BLACK
                rect = pygame.Rect(MARGIN+x*SQUARE_SIZE, y*SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE)
                pygame.draw.rect(window, cor, rect)
                if self.highlighted_pos == (x, y): pygame.draw.rect(window, GREEN, rect, 4)
                p = self.board[y][x]
                center = (MARGIN + x*SQUARE_SIZE + SQUARE_SIZE//2, y*SQUARE_SIZE + SQUARE_SIZE//2)
                if p in (1,3): pygame.draw.circle(window, RED, center, SQUARE_SIZE//2-10)
                if p in (2,4): pygame.draw.circle(window, BLUE, center, SQUARE_SIZE//2-10)
                if p in (3,4):
        ...
        for y in range(8):
            for x in range(8):
                ...
                if p==2:
                    for dx in(-1,1):
                        nx,ny=x+dx,y+1
                        if 0<=nx<8 and 0<=ny<8 and self.board[ny][nx]==0: movs.append(((x,y),(nx,ny)))
                    for dx in(-2,2):
                        nx,ny=x+dx,y+2;mx,my=x+dx//2,y+1
                        if 0<=nx<8 and 0<=ny<8 and self.board[ny][nx]==0 and self.board[my][mx] in(1,3): movs.append(((x,y),(nx,ny)))
                if p==4:
                    for dx,dy in[(-1,-1),(-1,1),(1,-1),(1,1)]:
                        if 0<=x+dx<8 and 0<=y+dy<8 and self.board[y+dy][x+dx]==0: movs.append(((x,y),(x+dx,y+dy)))
                    for dx,dy in[(-2,-2),(-2,2),(2,-2),(2,2)]:
                        mx,my=x+dx//2,y+dy//2;nx,ny=x+dx,y+dy
                        if 0<=nx<8 and 0<=ny<8 and self.board[ny][nx]==0 and self.board[my][mx] in(1,3): movs.append(((x,y),(nx,ny)))
        if movs:
            origem,destino=random.choice(movs);print(f'IA movendo de {origem} para {destino}')
            x0,y0=origem;x1,y1=destino;p=self.board[y0][x0]
            if abs(x1-x0)==2:
                mx,my=(x0+x1)//2,(y0+y1)//2;self.board[my][mx]=0;print(f'IA capturou em {(mx,my)}')
            self.board[y1][x1]=p;self.board[y0][x0]=0
            if p==2 and y1==7: self.board[y1][x1]=4;print(f'IA promoveu peça em {(x1,y1)}')
    ...
    def handle_move(self, comando):
        if comando=='cancelar' and self.highlighted_pos:
            ...
            print('Comando: reiniciar');self.initialize_board();self.highlighted_pos=None
        elif comando=='menu':
            ...
            if p in (1, 2):
                ...
                if abs(dx) == 1 and dy == dir_y and self.board[r][c] == 0:
                    ...
                    if self.mode == 1:
                        self.execute_blue_move()
                elif abs(dx) == 2 and dy == 2*dir_y and self.board[r][c] == 0:
                    mx, my = (ox + c)//2, (oy + r)//2
                    if self.board[my][mx] not in (p, 0):
                        ...
                        if self.mode == 1:
                            self.execute_blue_move()
            if p in (3, 4):
                if abs(dx) == 1 and abs(dy) == 1 and self.board[r][c] == 0:
                    ...
                    if self.mode == 1:
                        self.execute_blue_move()
                    return
                if abs(dx) == 2 and abs(dy) == 2 and self.board[r][c] == 0:
                    mx, my = (ox + c)//2, (oy + r)//2
                    if self.board[my][mx] not in (p, 0):
                        ...
                        for ddx, ddy in [(-2, -2), (-2, 2), (2, -2), (2, 2)]:
                            ...
                            if (0 <= nx < 8 and 0 <= ny < 8 and self.board[ny][nx] == 0 and
                                self.board[my2][mx2] in ((1, 3) if p == 4 else (2, 4))):
        ...
        while self.running:
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    ...
                    self.running = False
                if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1:
                    sel = ((e.pos[1] // SQUARE_SIZE),
                           ((e.pos[0] - MARGIN) // SQUARE_SIZE))
